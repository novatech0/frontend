...
export class HumidityChartComponent implements OnInit, OnChanges {
  ...
  private createChart(): void {
    ...
    // recalcular el generador según size actual
    ...
    this.svg = d3.select(element)
      ...
      .attr('viewBox', `-${this.strokeWidth} -${this.strokeWidth} ${this.size + this.strokeWidth * 2} ${this.size + this.strokeWidth * 2}`)
      .append('g')
  ...
  private updateChart(newValue: number): void {
    ...
    const strokeColor = newValue >= this.threshold ? '#00bcd4' : '#ff9800';
    this.pathForeground.style('stroke', strokeColor);
    ...
    this.textValue.transition()
      ...
      .tween('text', () => {
        ...
        return (t: number) => {
          this.textValue.text(`${Math.round(i(t))}%`);
        };
      });
  }
}
